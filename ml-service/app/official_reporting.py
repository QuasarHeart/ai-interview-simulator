...
class _GoogleReportModelConfig:
    model: str = os.getenv("REPORT_MODEL", os.getenv("GEMINI_MODEL", "gemini-2.5-flash"))
...
class _GoogleReportEngine:
    ...
    async def _generate_json(self, *, prompt_config: dict[str, Any], kwargs: dict[str, Any], schema: dict[str, Any]) -> dict[str, Any]:
    ...
    async def analyze_answer(self, req: AnalysisRequest) -> dict:
        ...
        return self._normalize_analysis_result(ai_result)
    ...
    async def generate_overall_report(self, req: ReportRequest) -> dict:
        ...
        return _ReportPromptOutput.model_validate(report).model_dump()
...
class OfficialReportService:
    ...
    async def build_round_results(self, context: Any, stage_results: list[Any]) -> list[RoundResultItem]:
        ...
        for stage_result in stage_results:
            ...
            for turn_record in self._extract_round_items(stage_result) or [stage_result]:
                ...
                try:
                    analysis = await engine.analyze_answer(analysis_request)
                    ...
                    round_results.append(self._fallback_round_result(round_id=round_id, stage=stage, reason=f"analysis failed: {exc}"))

        if not round_results:
            round_results.append(self._fallback_round_result(round_id=1, stage="intro", reason="no scored rounds collected"))
    ...
    async def generate_and_callback(self, context: Any, stage_results: list[Any]) -> dict[str, Any]:
        ...
        try:
            ...
            report_payload = await self._ensure_engine().generate_overall_report(report_request)
            logger.info(
